...
unsigned char login(unsigned char key, unsigned char reset_flag)
{
    ...
    if (i == 4) /* done entering the password */
    {
        ...
        {
            // clear screen and display login success
            clear_screen();
        ...
        else
        {
            ...
            else /* giving another attempt */
            {
                ...
                clcd_putch(attempts_rem + '0', LINE2(1));
...
void view_log(unsigned char key, char reset_index)
{
    ...
    else
    {
        ...
        if (key == SW5 && index < event_count) /* SW4 for moving upward */
        {
            index++; /*increment in position */
            clcd_print("                ", LINE2(0));
        }
        else if (key == SW4 && index > 0) /* SW5 for moving downward */
        {
